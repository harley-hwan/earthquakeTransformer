    ...
    def train(args):
        ...
        if args['mode'] == 'generator': 
            ...
            history = model.fit_generator(generator=training_generator,
                                          ...
                                          callbacks=callbacks, 
                                          epochs=args['epochs'],
                                          class_weight=['detector', 'picker_P', 'picker_S'])
...
def _split(args, save_dir):
    ...
    #
    random_idx = np.random.choice(len(df_local), 500)
    ev_list_local = df_local['trace_name'].to_numpy()[random_idx]
    random_idx = np.random.choice(len(df_noise), 500)
...
def _make_callback(args, save_models):
    ...
    callbacks = [checkpoint, lr_reducer, lr_scheduler, early_stopping_monitor, tensorboard_cb]
    return callbacks
